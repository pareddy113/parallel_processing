...
an n dimension hypercube using two (n-1) dimension hypercubes. I used this concept and used
...
performed.
3. The MPI_Gather is used to gather all the local sums of all the processes to the source node
(Process 0) to perform the final sum operation.
*/
...
main(int argc, char**argv)
{
    ...
    int world_rank1,a,m,rank, process_arr_length, j,sum=0, k, x, *par_sum_array, z,size, world_size1, par_sum=0, dim, *processes_per_dim, *period;
...
//Processor 1 creating an array of number and distributing it to the rest of the cube
    if(world_rank1 ==0)
...
//creating a communicator for the hypercube topology
    ...
    rec_time-=MPI_Wtime();
    MPI_Gather(&par_sum,1, MPI_INT, par_sum_array,1, MPI_INT,0, Hypercube);	//gather all the local sums and send it to processor 0
    rec_time+=MPI_Wtime();

//processor 0 receving the local sum from all the processors and performing final sum
    if(rank == 0)
    {
        arr_add_time-=MPI_Wtime();
        for(j=0; j<world_size1; j++)
        {
            sum+=par_sum_array[j];
            printf("Partial sum from process %d is= %d\n",j,par_sum_array[j]);
        }
